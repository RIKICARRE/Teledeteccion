%% Script para extraer, visualizar histogramas y obtener datos numéricos
% de las bandas B02, B03, B04, B08, B08A, B11 y B12 de las subcarpetas de "Fotos_T".
%
% Esto te permitirá analizar los histogramas y obtener valores numéricos
% (mínimo, máximo, percentil 1 y percentil 99) para cada banda, lo que puede ser
% útil para determinar cortes de cola adecuados que luego puedan aplicarse en las
% composiciones de True Color y False Color.

% Definir la ruta principal
rutaPrincipal = 'Fotos_T';

% Lista de subcarpetas (fechas)
subcarpetas = {
    '21-06-2017', ...
    '21-06-2019', ...
    '24-06-2024', ...
...
% Para cada subcarpeta procesamos las bandas
for i = 1:length(subcarpetas)
    ...
    if exist(archivoB02, 'file') && exist(archivoB03, 'file') && ...
       ...
       exist(archivoB12, 'file')
        ...
        % (Opcional) Crear una figura de histogramas
        figure('Name', ['Histogramas - ' subcarpetas{i}], 'NumberTitle','off');
        
        % Organizar los histogramas en una grilla de 2 filas x 4 columnas
        ...
        % Calcular y mostrar datos numéricos (mín, máx, 1er y 99 percentil) para cada banda:
        bandas = {'B02', 'B03', 'B04', 'B08', 'B08A', 'B11', 'B12'};
        datos = {B02, B03, B04, B08, B08A, B11, B12};
        fprintf('\n--- Estadísticas para %s ---\n', subcarpetas{i});
        for j = 1:length(bandas)
            bandData = double(datos{j});
            minVal = min(bandData(:));
            maxVal = max(bandData(:));
            p1 = prctile(bandData(:), 1);
            p99 = prctile(bandData(:), 99);
            fprintf('%s: min=%.2f, max=%.2f, 1%%=%.2f, 99%%=%.2f\n', bandas{j}, minVal, maxVal, p1, p99);
        end
        fprintf('--------------------------------------------\n\n');
    else
